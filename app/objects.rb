...
class Ai1 < Paddle
  ...
  def draw(args)
    y_center = @pos.y + (heigth * 0.5)
    if DEBUG
      args.outputs.primitives << { primitive_marker: :solid, x: @pos.x, y: y_center - @tolerance, w: @width,
                                   h: @tolerance * 2, r: 50, g: 50, b: 50 }
    end
    super
  end
...
class Ball
  ...
  def draw(args)
    args.outputs.solids << tertiary(args).merge(x: @pos.x, y: @pos.y, w: @width, h: @heigth)
  end
